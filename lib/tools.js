    ...
    do {
        ...
        for (const tool of response.tools) {
            ...
            if (definitions.has(publicName)) {
                throw new Error(`fitmeet-dsh-plugin(${opts.serverName}): server listed tool "${tool.name}" more than once — invalid tool list`);
            }
        ...
        if (cursor) {
            if (seenCursors.has(cursor) || seenCursors.size >= 99)
                throw new Error('FitMeet tool discovery pagination did not terminate; existing tools were preserved.');
