...
export function mcpGuidance(serverName) {
    ...
    if (DISTRIBUTION_LOCALE === 'zh-CN')
        return `用户需要找人、搜索需求与能力、读取组局、提醒、事项或反馈时，使用 ${prefix} 开头的工具。发布、开聊或发送消息需先 prepare；回执为 AUTOMATIC 时直接按原样标识提交并传 authorizationMode=AUTOMATIC，否则取得明确确认后再 confirm。OAuth 不等于业务确认。403 缺权限不等于登录过期，刷新不能增加权限。按用户语言自然简洁地回答，不展示内部状态枚举。组局写操作使用返回的网页入口。`;
