...
// Leave enough time for the transport-close event that proves the old
// generation is gone; timing out fails closed instead of overlapping clients.
 ...
 * @param reporter - Web-facing lifecycle and capability projection.
 * @returns Handle with a `ready` promise for startup-await and a `dispose` for teardown.
 */
export function startConnection(ctx, config, policy, oauth, reporter) {
    const label = `fitmeet-dsh-plugin(${config.serverName})`;
    const opts = {
        registrationFailure: 'contain',
        serverName: config.serverName,
        toolCallTimeoutMs: config.toolCallTimeoutMs,
        onCapabilities: capabilities => { reporter.setCapabilities(capabilities); },
    };
    ...
    /** Most recent failure retained for the Web status while reconnecting. */
    let lastFailure;
    ...
    function scheduleReconnect() {
        ...
        if (!policy.enabled) {
            ...
            reporter.setStatus('error', lastFailure);
            return;
        }
        ...
        reporter.setStatus('reconnecting', lastFailure);
    ...
    async function connectGeneration(startup) {
        reporter.setStatus('connecting');
        const generation = new Client({ name: 'fitmeet-dsh-plugin', version: '0.1.0' }, { capabilities: {} });
        ...
        try {
            await generation.connect(createTransport(config, oauth));
            if (hasClosed()) {
                attemptSettled = true;
                generationDown(generation);
                return;
            ...
            let failure = error;
            if (error instanceof UnauthorizedError && isCurrent(generation)) {
                try {
                    reporter.setStatus('authorizing');
                    await oauth.waitForAuthorization();
                    try {
                    ...
                    if (!quiesced) {
                        client = undefined;
                        clientClosed = undefined;
                        reporter.setStatus('error', 'OAuth transport did not close; reload the plugin to retry');
                        ctx.logger.error(`${label}: OAuth transport did not close within ${GENERATION_CLOSE_TIMEOUT_MS}ms — reconnect stopped; reload the plugin to retry`);
                        return;
                    }
                    await connectGeneration(startup);
                    return;
                }
                catch (authError) {
                    failure = authError;
            ...
            if (firstAttemptError === undefined)
                firstAttemptError = failure;
            lastFailure = failure;
            // Disposal clears current ownership before it closes the generation, so
            // only a live supervisor reports an attempt failure.
            if (isCurrent(generation))
                ctx.logger.warn(`${label}: connection attempt failed: ${String(failure)}`);
            try {
                await generation.close();
            }
            catch { /* transport already gone */ }
            ...
            if (!quiesced) {
                client = undefined;
                clientClosed = undefined;
                reporter.setStatus('error', 'Failed MCP generation did not close; reload the plugin to retry');
